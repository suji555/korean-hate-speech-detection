...
class make_dataloader:
    ...
    def mix_preprocess_dataframe(self, df):
        ...
        for c, l in zip(comments, label):
            if c in candidates and len(candidates) > 1:
                candidates.remove(c)
        ...
        submission.to_csv('/HDD/jangsj/korean-hate-speech-detection/mix_similar_low_all.csv',index=False)
    ...
    def mix_train_dataloader(self):
        ...
        df = self.read_data('/HDD/jangsj/korean-hate-speech-detection/mix_similar_low_all.csv')
        p = re.compile('[0-9]+')
        comment = []
...
for e in range(1, EPOCHS+1):
    ...
    if not best_val_f1 or val_f1 > best_val_f1:
        ...
        torch.save(model.state_dict(), '/home/jangsj/KoBERTScore/KoBERTScore/snapshot/txtclassificationlow.pt')
        print('[save model]')
        best_val_f1 = val_f1
